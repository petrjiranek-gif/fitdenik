...
import type { CoachPromptContext } from "@/lib/iron-man-2030/coach-prompt";
...
export type CoachLoadedData = {
  state: IronMan2030State;
  sessions: TrainingSession[];
  bodyEntries: BodyMeasurementEntry[];
  hrvEntries: HrvEntry[];
  promptContext: CoachPromptContext;
};

export async function loadCoachData(): Promise<CoachLoadedData | null> {
  ...
  if (trainingRes.error) {
    throw new Error(trainingRes.error.message);
  }
  ...
  const weightProgress = computeWeightProgress(bodyEntries);
  const phaseCompletion = computePhaseCompletion(sessions, phase, phase.id);
  const hrvTrend = computeHrvTrend(hrvEntries, now);

  const promptContext: CoachPromptContext = {
    settings: state.settings,
    phase,
    phaseCompletionPct: phaseCompletion.pct,
    currentWeightKg: weightProgress.current,
    date703: race703TargetDate(state.settings),
    date1406: race1406TargetDate(state.settings),
    hrvTrend,
    bodyEntries,
    sessions,
    checkIn: state.coachCheckIn,
    now,
  };

  return { state, sessions, bodyEntries, hrvEntries, promptContext };
}

export async function saveIronManState(state: IronMan2030State): Promise<string | null> {
  const supabase = getSupabaseServerClient();
